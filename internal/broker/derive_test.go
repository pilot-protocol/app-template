...
func TestDeriveKey_DeterministicAndUnique(t *testing.T) {
	...
	if first, again := DeriveKey(secret, 1, 0, a), DeriveKey(secret, 1, 0, a); first != again {
		t.Fatal("same (secret,version,caller) must derive the same key")
	}
	if DeriveKey(secret, 1, 0, a) == DeriveKey(secret, 1, 0, b) {
		t.Fatal("different callers must derive different keys")
	}
	if DeriveKey(secret, 1, 0, a) == DeriveKey(secret, 2, 0, a) {
		t.Fatal("different versions must derive different keys")
	}
	if DeriveKey(secret, 1, 0, a) == DeriveKey([]byte("other"), 1, 0, a) {
...
func TestValidateDerived_RoundTrip(t *testing.T) {
	...
	tok := DeriveKey(secret, 3, 0, caller)

	got, ok := ValidateDerived(map[byte][]byte{3: secret}, tok, 0)
	if !ok {
...
func TestValidateDerived_WrongSecretRejected(t *testing.T) {
	...
	tok := DeriveKey([]byte("real"), 1, 0, caller)
	if _, ok := ValidateDerived(map[byte][]byte{1: []byte("attacker")}, tok, 0); ok {
...
func TestValidateDerived_TamperRejected(t *testing.T) {
	...
	tok := DeriveKey(secret, 1, 0, caller)
	...
	if _, ok := ValidateDerived(map[byte][]byte{1: secret}, string(b), 0); ok {
...
func TestValidateDerived_ForgedCallerRejected(t *testing.T) {
	...
	tok := DeriveKey(secret, 1, 0, victim)
	...
	if _, ok := ValidateDerived(map[byte][]byte{1: secret}, forged, 0); ok {
...
func TestValidateDerived_RotationGraceWindow(t *testing.T) {
	...
	oldTok := DeriveKey(oldSecret, 1, 0, caller)
	newTok := DeriveKey(newSecret, 2, 0, caller)

	// During the grace window both versions are accepted.
	grace := map[byte][]byte{1: oldSecret, 2: newSecret}
	if _, ok := ValidateDerived(grace, oldTok, 0); !ok {
		t.Fatal("old-version token must validate during grace window")
	}
	if _, ok := ValidateDerived(grace, newTok, 0); !ok {
	...
	if _, ok := ValidateDerived(current, oldTok, 0); ok {
		t.Fatal("old-version token must be rejected after the grace window")
	}
	if _, ok := ValidateDerived(current, newTok, 0); !ok {
